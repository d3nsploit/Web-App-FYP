...
# Database connection
app.config['MYSQL_HOST'] = 'database-1.ceamamk7bxmm.ap-southeast-1.rds.amazonaws.com'
app.config['MYSQL_USER'] = 'scanme'
app.config['MYSQL_PASSWORD'] = 'scanme'
...
def result():
    ...
    cur.execute("INSERT INTO recent_scan (urls,status,date) VALUES (%s,%s,%s)",
                (get_url, prediction[0], get_time,))
...
def dashboard():
    ...
    if 'email' in session:
        ...
        if session['role'] == "admin":
            cur.execute(
                "SELECT urls,status,date from recent_scan order by id DESC limit 6")
            ...
            cur.execute(
                "SELECT count(id) from recent_scan where status = 'benign'")
            count_benign = cur.fetchone()['count(id)']

            cur.execute(
                "SELECT count(id) from recent_scan where status = 'malicious'")
...
def delreport():
    ...
    id = cur.fetchone()['id']
    cur.execute(
        "DELETE FROM report WHERE uid=%s and url=%s and date=%s", (id, url, date,))
    mysql.connection.commit()
...
def valreport():
    ...
    cur.execute("UPDATE report SET status = %s WHERE id=%s",
                (validate, rep_id,))
    mysql.connection.commit()
    return redirect(url_for('report'))
...
def editpic():
    ...
    if img_user and allowed_file(img_user.filename):
        ...
        cur.execute("UPDATE users SET image = %s WHERE email = %s",
                    (pic_bs64, session['email'],))
        mysql.connection.commit()
...
def context_processor():
    try:
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT fname, lname, email, username, image from users where email = %s", (session['email'],))
        user_info = cur.fetchone()
        cur.execute("SELECT image from users where email = %s",
                    (session['email'],))
